...
bool DB_Local::abrir_DB(){
    ...
    sqlstr = "CREATE TABLE IF NOT EXISTS registro_usuario (`id_usuariorg` INTEGER NOT NULL PRIMARY KEY AUTOINCREMENT, "
             " `user_name` TEXT NOT NULL,`fecha` TEXT NOT NULL, `hora` TEXT NOT NULL, UNIQUE(user_name));";
             CREAR_TABLA(sqlstr);
...
bool DB_Local::Guardar_datos_sensores( double temperatura, double humedad, double direccion_del_viento, double precipitacion, double velocidad_del_viento,
    ...
    sqlsentence << "date('now')" << " , " << "time('now')" << " , " << intensidad_de_la_luz << " , " << temperatura<< " , " << humedad << " , " << velocidad_del_viento<<" , " << direccion_del_viento<< " , " << precipitacion<< ");";
...
int DB_Local::alerta_recuperar(std::string &fechahoy,double &temperaturahoy, double &humedadhoy,std::string &fechaayer,double &temperaturaayer, double &humedadayer){
    ...
    std::string tmpAyer[3]; 

    std::stringstream sql;
    sql << "SELECT fecha, temperatura, humedad FROM alerta_temprana WHERE fecha = date('now','-1 day');";
    rc = sqlite3_exec( _db, sql.str().c_str(), rellenar_string, (void*)&tmpAyer, &mesg );
    ...
    fechaayer = tmpAyer[0];
    temperaturaayer = atof(tmpAyer[1].c_str());
    humedadayer = atof(tmpAyer[2].c_str());


    std::string tmpHoy[3];
    sql.str("");
    sql << "SELECT fecha, temperatura, humedad FROM alerta_temprana WHERE fecha = date('now');";
    rc = sqlite3_exec( _db, sql.str().c_str(), rellenar_string, (void*)&tmpHoy, &mesg );
    ...
    fechahoy = tmpHoy[0];
    temperaturahoy = atof(tmpHoy[1].c_str());
    humedadhoy = atof(tmpHoy[2].c_str());
